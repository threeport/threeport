...
import (
	...
	azure "github.com/threeport/threeport/internal/azure"
...
func main() {
	// flags
	var azureAksKubernetesRuntimeInstanceConcurrentReconciles = flag.Int(
		"azure-aks-kubernetes-runtime-instance-concurrent-reconciles",
		1,
	...
	kvConfig := natsgo.KeyValueConfig{
		Bucket:      azure.LockBucketName,
		Description: azure.LockBucketDescr,
		TTL:         time.Minute * 20,
	}
	kv, err := controller.CreateLockBucketIfNotExists(js, &kvConfig)
	if err != nil {
		log.Error(err, "failed to bind to JetStream key-value locking bucket", "lockBucketName", azure.LockBucketName)
		os.Exit(1)
	}

	// check to ensure azure stream has been created by API
	azureStreamNameFound := false
	for stream := range js.StreamNames() {
		if stream == notif.AzureStreamName {
			azureStreamNameFound = true
		}
	}
	if !azureStreamNameFound {
		log.Error(errors.New("JetStream stream not found"), "failed to find stream with azure stream name", "azureStreamName", notif.AzureStreamName)
		os.Exit(1)
	}
	...
	reconcilerConfigs = append(reconcilerConfigs, controller.ReconcilerConfig{
		ConcurrentReconciles: *azureAksKubernetesRuntimeInstanceConcurrentReconciles,
	...
	for _, r := range reconcilerConfigs {
		...
		js.AddConsumer(notif.AzureStreamName, &natsgo.ConsumerConfig{
		...
		sub, err := js.PullSubscribe(r.NotifSubject, consumer, natsgo.BindStream(notif.AzureStreamName))
		if err != nil {
		...
		reconciler := controller.Reconciler{
			...
			EventsRecorder: &event.EventRecorder{
				APIClient:           apiClient,
				APIServer:           *apiServer,
				ReportingController: "AzureController",
			},
			JetStreamContext: js,
			KeyValue:         kv,
			Log:              &log,
	...
	log.Info("azure controller shutting down")
	os.Exit(0)
}
func showHelpAndExit(exitCode int) {
	fmt.Printf("Usage: threeport-azure-controller [options]\n")
	fmt.Println("options:")
	flag.PrintDefaults()
	os.Exit(exitCode)
}

...
var GetWorkloadDefinitionsCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		v0WorkloadDefinitions, err := client_v0.GetWorkloadDefinitions(apiClient, apiEndpoint)
		if err != nil {
			cli.Error("failed to retrieve workload definition instances", err)
			os.Exit(1)
		}

		// write the output
		if len(*v0WorkloadDefinitions) == 0 {
			cli.Info(fmt.Sprintf(
				"No workloadDefinitions currently managed by %s threeport control plane",
				requestedControlPlane,
			))
			os.Exit(0)
		}
		if err := outputGetWorkloadDefinitionsCmd(
			v0WorkloadDefinitions,
			apiClient,
			apiEndpoint,
		); err != nil {
			cli.Error("failed to produce output", err)
...
var GetWorkloadInstancesCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		v0WorkloadInstances, err := client_v0.GetWorkloadInstances(apiClient, apiEndpoint)
		if err != nil {
			cli.Error("failed to retrieve workload instance instances", err)
			os.Exit(1)
		}
		v1WorkloadInstances, err := client_v1.GetWorkloadInstances(apiClient, apiEndpoint)
		if err != nil {
		...
		if len(*v0WorkloadInstances) == 0 && len(*v1WorkloadInstances) == 0 {
			cli.Info(fmt.Sprintf(
				"No workloadInstances currently managed by %s threeport control plane",
				requestedControlPlane,
			))
			os.Exit(0)
		}

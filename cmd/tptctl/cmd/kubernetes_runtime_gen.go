...
var GetKubernetesRuntimeDefinitionsCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		v0KubernetesRuntimeDefinitions, err := client_v0.GetKubernetesRuntimeDefinitions(apiClient, apiEndpoint)
		if err != nil {
			cli.Error("failed to retrieve kubernetes runtime definition instances", err)
			os.Exit(1)
		}

		// write the output
		if len(*v0KubernetesRuntimeDefinitions) == 0 {
			cli.Info(fmt.Sprintf(
				"No kubernetesRuntimeDefinitions currently managed by %s threeport control plane",
				requestedControlPlane,
			))
			os.Exit(0)
		}
		if err := outputGetKubernetesRuntimeDefinitionsCmd(
			v0KubernetesRuntimeDefinitions,
			apiClient,
			apiEndpoint,
		); err != nil {
			cli.Error("failed to produce output", err)
...
var GetKubernetesRuntimeInstancesCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		v0KubernetesRuntimeInstances, err := client_v0.GetKubernetesRuntimeInstances(apiClient, apiEndpoint)
		if err != nil {
			cli.Error("failed to retrieve kubernetes runtime instance instances", err)
			os.Exit(1)
		}
		...
		if len(*v0KubernetesRuntimeInstances) == 0 {
			cli.Info(fmt.Sprintf(
				"No kubernetesRuntimeInstances currently managed by %s threeport control plane",
				requestedControlPlane,
			))
			os.Exit(0)
		}

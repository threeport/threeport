...
var DeleteControlPlaneInstanceCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		cliArgs.AuthEnabled, err = threeportConfig.GetThreeportAuthEnabled(cliArgs.ControlPlaneName)
		if err != nil {
			cli.Error("failed to determine if auth is enabled on threeport API", err)
			os.Exit(1)
		}
		ca, clientCertificate, clientPrivateKey, err := threeportConfig.GetThreeportCertificatesForControlPlane(cliArgs.ControlPlaneName)
		if err != nil {

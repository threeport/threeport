...
var GetHelmWorkloadDefinitionsCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		v0HelmWorkloadDefinitions, err := client_v0.GetHelmWorkloadDefinitions(apiClient, apiEndpoint)
		if err != nil {
			cli.Error("failed to retrieve helm workload definition instances", err)
			os.Exit(1)
		}

		// write the output
		if len(*v0HelmWorkloadDefinitions) == 0 {
			cli.Info(fmt.Sprintf(
				"No helmWorkloadDefinitions currently managed by %s threeport control plane",
				requestedControlPlane,
			))
			os.Exit(0)
		}
		if err := outputGetHelmWorkloadDefinitionsCmd(
			v0HelmWorkloadDefinitions,
			apiClient,
			apiEndpoint,
		); err != nil {
			cli.Error("failed to produce output", err)
...
var GetHelmWorkloadInstancesCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		v0HelmWorkloadInstances, err := client_v0.GetHelmWorkloadInstances(apiClient, apiEndpoint)
		if err != nil {
			cli.Error("failed to retrieve helm workload instance instances", err)
			os.Exit(1)
		}
		...
		if len(*v0HelmWorkloadInstances) == 0 {
			cli.Info(fmt.Sprintf(
				"No helmWorkloadInstances currently managed by %s threeport control plane",
				requestedControlPlane,
			))
			os.Exit(0)
		}

...
var GetControlPlaneDefinitionsCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		v0ControlPlaneDefinitions, err := client_v0.GetControlPlaneDefinitions(apiClient, apiEndpoint)
		if err != nil {
			cli.Error("failed to retrieve control plane definition instances", err)
			os.Exit(1)
		}

		// write the output
		if len(*v0ControlPlaneDefinitions) == 0 {
			cli.Info(fmt.Sprintf(
				"No controlPlaneDefinitions currently managed by %s threeport control plane",
				requestedControlPlane,
			))
			os.Exit(0)
		}
		if err := outputGetControlPlaneDefinitionsCmd(
			v0ControlPlaneDefinitions,
			apiClient,
			apiEndpoint,
		); err != nil {
			cli.Error("failed to produce output", err)
...
var GetControlPlaneInstancesCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		v0ControlPlaneInstances, err := client_v0.GetControlPlaneInstances(apiClient, apiEndpoint)
		if err != nil {
			cli.Error("failed to retrieve control plane instance instances", err)
			os.Exit(1)
		}
		...
		if len(*v0ControlPlaneInstances) == 0 {
			cli.Info(fmt.Sprintf(
				"No controlPlaneInstances currently managed by %s threeport control plane",
				requestedControlPlane,
			))
			os.Exit(0)
		}

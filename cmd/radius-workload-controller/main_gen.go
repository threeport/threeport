...
import (
	...
	radiusworkload "github.com/threeport/threeport/internal/radius-workload"
...
func main() {
	// flags
	var radiusWorkloadDefinitionConcurrentReconciles = flag.Int(
		"RadiusWorkloadDefinition-concurrent-reconciles",
		1,
		"Number of concurrent reconcilers to run for radius workload definitions",
	)
	var radiusWorkloadInstanceConcurrentReconciles = flag.Int(
		"RadiusWorkloadInstance-concurrent-reconciles",
		1,
		"Number of concurrent reconcilers to run for radius workload instances",
	)
	...
	kvConfig := natsgo.KeyValueConfig{
		Bucket:      radiusworkload.LockBucketName,
		Description: radiusworkload.LockBucketDescr,
		TTL:         time.Minute * 20,
	}
	kv, err := controller.CreateLockBucketIfNotExists(js, &kvConfig)
	if err != nil {
		log.Error(err, "failed to bind to JetStream key-value locking bucket", "lockBucketName", radiusworkload.LockBucketName)
		os.Exit(1)
	}

	// check to ensure radius-workload stream has been created by API
	radiusWorkloadStreamNameFound := false
	for stream := range js.StreamNames() {
		if stream == v0.RadiusWorkloadStreamName {
			radiusWorkloadStreamNameFound = true
		}
	}
	if !radiusWorkloadStreamNameFound {
		log.Error(errors.New("JetStream stream not found"), "failed to find stream with radius-workload stream name", "radiusWorkloadStreamName", v0.RadiusWorkloadStreamName)
		os.Exit(1)
	}
	...
	reconcilerConfigs = append(reconcilerConfigs, controller.ReconcilerConfig{
		ConcurrentReconciles: *radiusWorkloadDefinitionConcurrentReconciles,
		Name:                 "RadiusWorkloadDefinitionReconciler",
		NotifSubject:         v0.RadiusWorkloadDefinitionSubject,
		ObjectType:           v0.ObjectTypeRadiusWorkloadDefinition,
		ReconcileFunc:        radiusworkload.RadiusWorkloadDefinitionReconciler,
	})
	reconcilerConfigs = append(reconcilerConfigs, controller.ReconcilerConfig{
		ConcurrentReconciles: *radiusWorkloadInstanceConcurrentReconciles,
		Name:                 "RadiusWorkloadInstanceReconciler",
		NotifSubject:         v0.RadiusWorkloadInstanceSubject,
		ObjectType:           v0.ObjectTypeRadiusWorkloadInstance,
		ReconcileFunc:        radiusworkload.RadiusWorkloadInstanceReconciler,
	})
	...
	for _, r := range reconcilerConfigs {
		...
		js.AddConsumer(v0.RadiusWorkloadStreamName, &natsgo.ConsumerConfig{
		...
		sub, err := js.PullSubscribe(r.NotifSubject, consumer, natsgo.BindStream(v0.RadiusWorkloadStreamName))
		if err != nil {
	...
	log.Info(
		"radius-workload controller started",
		"version", version.GetVersion(),
		"controllerID", controllerID.String(),
		"NATSConnection", natsConn,
		"lockBucketName", radiusworkload.LockBucketName,
	)
	...
	log.Info("radius-workload controller shutting down")
	os.Exit(0)
}
func showHelpAndExit(exitCode int) {
	fmt.Printf("Usage: threeport-radius-workload-controller [options]\n")
	fmt.Println("options:")
	flag.PrintDefaults()
	os.Exit(exitCode)
}

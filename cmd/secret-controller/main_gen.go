...
func main() {
	...
	for stream := range js.StreamNames() {
		if stream == v0.SecretStreamName {
			secretStreamNameFound = true
		}
	}
	if !secretStreamNameFound {
		log.Error(errors.New("JetStream stream not found"), "failed to find stream with secret stream name", "secretStreamName", v0.SecretStreamName)
		os.Exit(1)
	}

...
func main() {
	...
	for stream := range js.StreamNames() {
		if stream == v0.HelmWorkloadStreamName {
			helmWorkloadStreamNameFound = true
		}
	}
	if !helmWorkloadStreamNameFound {
		log.Error(errors.New("JetStream stream not found"), "failed to find stream with helm-workload stream name", "helmWorkloadStreamName", v0.HelmWorkloadStreamName)
		os.Exit(1)
	}
	...
	for _, r := range reconcilerConfigs {
		...
		js.AddConsumer(v0.HelmWorkloadStreamName, &natsgo.ConsumerConfig{
		...
		sub, err := js.PullSubscribe(r.NotifSubject, consumer, natsgo.BindStream(v0.HelmWorkloadStreamName))
		if err != nil {

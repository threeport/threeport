...
func main() {
	...
	for stream := range js.StreamNames() {
		if stream == v0.ControlPlaneStreamName {
			controlPlaneStreamNameFound = true
		}
	}
	if !controlPlaneStreamNameFound {
		log.Error(errors.New("JetStream stream not found"), "failed to find stream with control-plane stream name", "controlPlaneStreamName", v0.ControlPlaneStreamName)
		os.Exit(1)
	}
	...
	for _, r := range reconcilerConfigs {
		...
		js.AddConsumer(v0.ControlPlaneStreamName, &natsgo.ConsumerConfig{
		...
		sub, err := js.PullSubscribe(r.NotifSubject, consumer, natsgo.BindStream(v0.ControlPlaneStreamName))
		if err != nil {

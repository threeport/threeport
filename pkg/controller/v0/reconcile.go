...
func (r *Reconciler) UnlockAndRequeue(
	...
	notifPayload *[]byte,
	requeueDelay int64,
	reconcileTerminated chan bool,
) {
	if ok := r.ReleaseLock(object, reconcileTerminated); !ok {
		r.Log.V(1).Info(
...
func (r *Reconciler) ReleaseLock(object v0.APIObject, reconcileTerminated chan bool) bool {
	lockKey := r.lockKey(object.GetID())

...
func (h Handler) AddProfile(c echo.Context) error {
	...
	if err := apiserver_lib.AfterCommitCreate(h.DB, &profile); err != nil {
...
func (h Handler) ReplaceProfile(c echo.Context) error {
	...
	if err := crdbgorm.ExecuteTx(
		c.Request().Context(), h.DB, nil,
		func(tx *gorm.DB) error {
			return tx.Scopes(apiserver_lib.QueryScopes(c)...).Session(&gorm.Session{FullSaveAssociations: false}).Omit("CreatedAt", "DeletedAt").Save(&updatedProfile).Error
		},
	); err != nil {
...
func (h Handler) DeleteProfile(c echo.Context) error {
	...
	if err := crdbgorm.ExecuteTx(
		c.Request().Context(), h.DB, nil,
		func(tx *gorm.DB) error {
			return tx.Scopes(apiserver_lib.QueryScopes(c)...).Delete(&profile).Error
		},
	); err != nil {
	...
	if err := apiserver_lib.AfterCommitDelete(h.DB, &profile); err != nil {
		h.Logger.Error("handler error: error reconciling process state after commit", zap.Error(err))
		return apiserver_lib.ResponseStatus500(c, nil, err, objectType)
	}
...
func (h Handler) AddTier(c echo.Context) error {
	...
	// rely on it. A persist hook cannot do this: it runs inside the
	// transaction, so it would act on a write that may never commit.
	if err := apiserver_lib.AfterCommitCreate(h.DB, &tier); err != nil {
		h.Logger.Error("handler error: error reconciling process state after commit", zap.Error(err))
		return apiserver_lib.ResponseStatus500(c, nil, err, objectType)
	}
...
func (h Handler) ReplaceTier(c echo.Context) error {
	...
	if err := crdbgorm.ExecuteTx(
		c.Request().Context(), h.DB, nil,
		func(tx *gorm.DB) error {
			return tx.Scopes(apiserver_lib.QueryScopes(c)...).Session(&gorm.Session{FullSaveAssociations: false}).Omit("CreatedAt", "DeletedAt").Save(&updatedTier).Error
		},
	); err != nil {
		h.Logger.Error("handler error: error persisting object", zap.Error(err))
		// check if this is a custom HTTP error with specific status code
...
func (h Handler) DeleteTier(c echo.Context) error {
	...
	if err := crdbgorm.ExecuteTx(
		c.Request().Context(), h.DB, nil,
		func(tx *gorm.DB) error {
			return tx.Scopes(apiserver_lib.QueryScopes(c)...).Delete(&tier).Error
		},
	); err != nil {
		h.Logger.Error("handler error: error deleting object", zap.Error(err))
		// surface BlockedDeleteError from gorm hook - sole blocking check for non-reconciled types
	...
	// row before answering. A persist hook cannot do this: a rollback
	// would have dropped state for a row that survived.
	if err := apiserver_lib.AfterCommitDelete(h.DB, &tier); err != nil {
		h.Logger.Error("handler error: error reconciling process state after commit", zap.Error(err))
		return apiserver_lib.ResponseStatus500(c, nil, err, objectType)
	}

...
func (h Handler) AddHelmWorkloadDefinition(c echo.Context) error {
	...
	if err := apiserver_lib.AfterCommitCreate(h.DB, &helmWorkloadDefinition); err != nil {
...
func (h Handler) UpdateHelmWorkloadDefinition(c echo.Context) error {
	...
	if api_v0.ReconciliationUpdateNotifiable(prevReconciliation, existingHelmWorkloadDefinition.Reconciliation) {
...
func (h Handler) DeleteHelmWorkloadDefinition(c echo.Context) error {
	...
	if err := apiserver_lib.AfterCommitDelete(h.DB, &helmWorkloadDefinition); err != nil {
		h.Logger.Error("handler error: error reconciling process state after commit", zap.Error(err))
		return apiserver_lib.ResponseStatus500(c, nil, err, fullyQualifiedType)
	}
...
func (h Handler) AddHelmWorkloadInstance(c echo.Context) error {
	...
	// rely on it. A persist hook cannot do this: it runs inside the
	// transaction, so it would act on a write that may never commit.
	if err := apiserver_lib.AfterCommitCreate(h.DB, &helmWorkloadInstance); err != nil {
		h.Logger.Error("handler error: error reconciling process state after commit", zap.Error(err))
		return apiserver_lib.ResponseStatus500(c, nil, err, fullyQualifiedType)
	}
...
func (h Handler) UpdateHelmWorkloadInstance(c echo.Context) error {
	...
	// notify controller if the update is notifiable
	if api_v0.ReconciliationUpdateNotifiable(prevReconciliation, existingHelmWorkloadInstance.Reconciliation) {
...
func (h Handler) DeleteHelmWorkloadInstance(c echo.Context) error {
	...
	// row before answering. A persist hook cannot do this: a rollback
	// would have dropped state for a row that survived.
	if err := apiserver_lib.AfterCommitDelete(h.DB, &helmWorkloadInstance); err != nil {
		h.Logger.Error("handler error: error reconciling process state after commit", zap.Error(err))
		return apiserver_lib.ResponseStatus500(c, nil, err, fullyQualifiedType)
	}

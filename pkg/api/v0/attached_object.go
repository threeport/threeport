...
type AttachedObjectReference struct {
	...
	ObjectType *string `validate:"required" gorm:"not null;uniqueIndex:idx_attached_object_unique;uniqueIndex:idx_aor_marries_base,where:relationship = 'marries' AND deleted_at IS NULL;uniqueIndex:idx_aor_owns_base,where:relationship = 'owns' AND deleted_at IS NULL"`

	// ObjectID is the database ID of the base object.
	ObjectID *uint `validate:"required" gorm:"not null;uniqueIndex:idx_attached_object_unique;uniqueIndex:idx_aor_marries_base,where:relationship = 'marries' AND deleted_at IS NULL;uniqueIndex:idx_aor_owns_base,where:relationship = 'owns' AND deleted_at IS NULL"`
	...
	AttachedObjectType *string `validate:"required" gorm:"not null;uniqueIndex:idx_attached_object_unique;uniqueIndex:idx_aor_marries_attached,where:relationship = 'marries' AND deleted_at IS NULL"`

	// AttachedObjectID is the database ID of the attaching object.
	AttachedObjectID *uint `validate:"required" gorm:"not null;uniqueIndex:idx_attached_object_unique;uniqueIndex:idx_aor_marries_attached,where:relationship = 'marries' AND deleted_at IS NULL"`
	...
	Relationship *Relationship `validate:"optional" gorm:"default:'describes'"`
}

...
func GetControlPlaneDefinitions(apiClient *http.Client, apiAddr string) (*[]v0.ControlPlaneDefinition, error) {
	...
	for !allPagesReceived {
		url := fmt.Sprintf("%s%s", apiAddr, v0.PathControlPlaneDefinitions)
		if queryId != "" {
			url = fmt.Sprintf("%s%s?queryid=%s&cursor=%d", apiAddr, v0.PathControlPlaneDefinitions, queryId, nextCursor)
		}
...
func GetControlPlaneDefinitionByName(apiClient *http.Client, apiAddr, name string) (*v0.ControlPlaneDefinition, error) {
	...
	case len(controlPlaneDefinitions) < 1:
		return &v0.ControlPlaneDefinition{}, client_lib.ErrObjectNotFound
	case len(controlPlaneDefinitions) > 1:
		return &v0.ControlPlaneDefinition{}, fmt.Errorf("more than one control plane definition with name %s returned", name)
	}
...
func GetControlPlaneInstances(apiClient *http.Client, apiAddr string) (*[]v0.ControlPlaneInstance, error) {
	...
	queryId := ""
	for !allPagesReceived {
		url := fmt.Sprintf("%s%s", apiAddr, v0.PathControlPlaneInstances)
		if queryId != "" {
			url = fmt.Sprintf("%s%s?queryid=%s&cursor=%d", apiAddr, v0.PathControlPlaneInstances, queryId, nextCursor)
		}
...
func GetControlPlaneInstanceByName(apiClient *http.Client, apiAddr, name string) (*v0.ControlPlaneInstance, error) {
	...
	case len(controlPlaneInstances) < 1:
		return &v0.ControlPlaneInstance{}, client_lib.ErrObjectNotFound
	case len(controlPlaneInstances) > 1:
		return &v0.ControlPlaneInstance{}, fmt.Errorf("more than one control plane instance with name %s returned", name)
	}

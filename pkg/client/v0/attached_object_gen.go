...
func GetAttachedObjectReferences(apiClient *http.Client, apiAddr string) (*[]v0.AttachedObjectReference, error) {
	...
	for !allPagesReceived {
		url := fmt.Sprintf("%s%s", apiAddr, v0.PathAttachedObjectReferences)
		if queryId != "" {
			url = fmt.Sprintf("%s%s?queryid=%s&cursor=%d", apiAddr, v0.PathAttachedObjectReferences, queryId, nextCursor)
		}
...
func GetAttachedObjectReferenceByID(apiClient *http.Client, apiAddr string, id uint) (*v0.AttachedObjectReference, error) {
	...
	if len(response.Data) == 0 {
		return &attachedObjectReference, errors.New("threeport API returned no object in response data")
	}

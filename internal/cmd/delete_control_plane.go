...
func DeleteControlPlane(args *config.CLIArgs) error {
	...
	for _, instance := range threeportConfig.Instances {
		if instance.Name == args.InstanceName {
	...
	if !threeportInstanceConfigExists {
		cli.Error("failed to find threeport instance config",
			errors.New(fmt.Sprintf(
				"config for threeport instance with name %s not found", args.InstanceName)))
		os.Exit(1)
	}
	...
	config.DeleteThreeportConfigInstance(threeportConfig, args.InstanceName)

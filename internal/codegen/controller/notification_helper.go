...
func (cc *ControllerConfig) NotificationHelper() error {
	...
	f.HeaderComment("+threeport-codegen route-exclude")
	f.HeaderComment("+threeport-codegen database-exclude")

	objects := &Statement{}
	for _, name := range models.SetReconcileCheckTypeNames() {
	...
	if err != nil {
		return fmt.Errorf("failed open file to write generated code for notification helper: %w", err)
	}
	defer file.Close()

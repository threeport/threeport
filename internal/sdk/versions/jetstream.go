...
func (gvc *GlobalVersionConfig) InitJetStreamContext(sdkConfig *sdk.ApiObjectConfig, modulePath string) error {
	...
	jetstramInitPath := filepath.Join("cmd", "rest-api", "util")
	if _, err := os.Stat(jetstramInitPath); errors.Is(err, os.ErrNotExist) {
		if err := os.MkdirAll(jetstramInitPath, 0755); err != nil {
			return fmt.Errorf("could not create directores for controller stream initialization: %s, %w", jetstramInitPath, err)
		}
	}

	// write code to file
	controllerStreamFilePath := filepath.Join(jetstramInitPath, "controller_stream_gen.go")
	file, err := os.OpenFile(controllerStreamFilePath, os.O_CREATE|os.O_WRONLY|os.O_TRUNC, 0644)
	if err != nil {
		return fmt.Errorf("failed to open file to write generated code for controller stream initialization: %w", err)
	}
	defer file.Close()
	if err := f.Render(file); err != nil {
		return fmt.Errorf("failed to render generated source code for controller stream initialization: %w", err)
	}
	fmt.Println("code generation complete for controller stream initialization")

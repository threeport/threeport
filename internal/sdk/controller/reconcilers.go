					...
					Default().BlockFunc(func(g *jen.Group) {
						...
						g.Var().Id(strcase.ToLowerCamel(obj.Name)).Qual(
							fmt.Sprintf(
								...
								sdk.GetLatestObjectVersion(obj.Name),
							),
							obj.Name,
						)
								...
								Id(fmt.Sprintf(
									...
									Id("Common"): Qual(
										...
										Id("ID"): Qual(
											"github.com/threeport/threeport/pkg/util/v0",
											"Ptr",
										).Call(Id(strcase.ToLowerCamel(obj.Name)).Dot("GetId").Call()),
									}),
...
func (cc *ControllerConfig) ExtensionReconcilers(modulePath string) error {
	for _, obj := range cc.ReconciledObjects {
		...
		f.ImportAlias("github.com/threeport/threeport/pkg/api", "tpapi")
				...
				Select().Block(
					...
					Default().BlockFunc(func(g *jen.Group) {
						...
						g.Switch(Id("notif").Dot("ObjectVersion")).BlockFunc(func(h *Group) {
							...
							h.Default().Block(
								Id("log").Dot("Error").Call(
									Id("err"), Lit("received unrecognized version of wordpress definition object"),
								),
								Id("r").Dot("RequeueRaw").Call(Id("msg")),
								Id("log").Dot("V").Call(Lit(1)).Dot("Info").Call(
									Lit("wordpress definition reconciliation requeued with identical payload and fixed delay"),
								),
								Continue(),
							)
						})
						...
						g.Switch(Id("notif").Dot("Operation")).BlockFunc(func(h *Group) {
							...
							for op, opVars := range ops {
								h.Case(Qual(
									...
									h.Switch(Id(varObjectName).Dot("Version").Call()).BlockFunc(func(j *Group) {
										for _, version := range obj.Versions {
										...
										j.Default().Block(
											Id("log").Dot("Error").Call(
												Id("err"),
												Lit("unrecognized version of wordpress definition encountered for creation"),
											),
											Id("r").Dot("UnlockAndRequeue").Call(
												Id(varObjectName),
												Id("requeueDelay"),
...
// ReconcilerCrudFuncs generates the scaffolding for the CUD functions in the
// reconciler that contain the business logic for reconciling object state.
func (cc *ControllerConfig) ReconcilerCudFuncs() error {
	return nil
}

// ExtensionReconcilerCrudFuncs generates the scaffolding for the CUD functions in the
// reconciler that contain the business logic for reconciling object state.
func (cc *ControllerConfig) ExtensionReconcilerCudFuncs(modulePath string) error {
	for _, obj := range cc.ReconciledObjects {
		f := NewFile(cc.PackageName)
		f.HeaderComment("generated by 'threeport-sdk gen' for controller scaffolding - do not edit")
		...
		genFilename := fmt.Sprintf("%s.go", strcase.ToSnake(obj.Name))
		genFilepath := filepath.Join(controllerInternalPackagePath(cc.ShortName), genFilename)
		if _, err := os.Stat(genFilepath); errors.Is(err, os.ErrNotExist) {
			file, err := os.OpenFile(genFilepath, os.O_CREATE|os.O_WRONLY|os.O_TRUNC, 0644)
			if err != nil {
				return fmt.Errorf("failed to open file to write generated code for %s reconciler CUD functions: %w", obj.Name, err)
			}
			defer file.Close()
			if err := f.Render(file); err != nil {
				return fmt.Errorf("failed to render generated source code for %s reconciler CUD functions: %w", obj.Name, err)
			}
			fmt.Printf("code generation complete for %s reconciler CUD functions\n", obj.Name)
		} else {
			fmt.Printf("%s reconciler CUD functions already exist - not regenerated\n", obj.Name)
		}
	}
...
func (cc *ControllerConfig) GetLatestObject(g *jen.Group, obj string, moduleOverride *string) {
	...
	g.Id(fmt.Sprintf(
		...
		Line().Id("r").Dot("APIClient"),
		Line().Id("r").Dot("APIServer"),
		Line().Id(strcase.ToLowerCamel(obj)).Dot("GetId").Call(),
		Line(),
	)
	g.Line()
	...
	g.If(Qual("errors", "Is").Call(Id("err"), Qual(
		...
		Id("r").Dot("ReleaseLock").Call(Id(strcase.ToLowerCamel(obj)), Id("lockReleased"), Id("msg"), Lit(true)),
		Continue(),
	)
	g.If(Id("err").Op("!=").Nil()).Block(
		Id("log").Dot("Error").Call(Id("err"), Lit(fmt.Sprintf(
		...
		Id("r").Dot("UnlockAndRequeue").Call(Id(strcase.ToLowerCamel(obj)), Id("requeueDelay"), Id("lockReleased"), Id("msg")),
		Continue(),
	)
	g.Id(strcase.ToLowerCamel(obj)).Op("=").Id(fmt.Sprintf(
		"latest%s",
		obj,
	))
}

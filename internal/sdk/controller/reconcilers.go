...
func (cc *ControllerConfig) Reconcilers() error {
	for _, obj := range cc.ReconciledObjects {
		...
		f.HeaderComment("generated by 'threeport-sdk gen controller' - do not edit")
...
func (cc *ControllerConfig) ReconcilerFunctions() error {
	for _, obj := range cc.ReconciledObjects {
		...
		recFuncFileName := fmt.Sprintf("%s.go", strcase.ToSnake(obj))
		...
		if os.IsNotExist(err) {
			// Generate the file for reconciler functions the user will maintain
			f := NewFile(cc.PackageName)
			f.HeaderComment("generated by 'threeport-sdk gen controller'")
			f.ImportAlias("github.com/threeport/threeport/pkg/controller/v0", "controller")
			f.ImportAlias("github.com/threeport/threeport/pkg/api/v0", "v0")

			f.Comment(fmt.Sprintf("%[1]sReconciler reconciles system state when a %[1]s", obj))
			f.Comment("is created.")
			f.Func().Id(fmt.Sprintf("%sCreated", strcase.ToLowerCamel(obj))).Params(
				Id("r").Op("*").Qual(
					"github.com/threeport/threeport/pkg/controller/v0",
					"Reconciler",
				),
				Id(strcase.ToLowerCamel(obj)).Op("*").Qual(
					"github.com/threeport/threeport/pkg/api/v0",
					obj,
				),
				Id("log").Op("*").Qual(
			...
			f.Func().Id(fmt.Sprintf("%sUpdated", strcase.ToLowerCamel(obj))).Params(
				Id("r").Op("*").Qual(
					"github.com/threeport/threeport/pkg/controller/v0",
					"Reconciler",
				),
			...
			f.Func().Id(fmt.Sprintf("%sDeleted", strcase.ToLowerCamel(obj))).Params(
				Id("r").Op("*").Qual(
					"github.com/threeport/threeport/pkg/controller/v0",
					"Reconciler",
				),

...
func (cc *ControllerConfig) getQualifiedClientPath() string {
	...
	case "v0":
		return ""
	default:
		return "github.com/threeport/threeport/pkg/client/v0"
	}
}
...
func (cc *ControllerConfig) ClientLib() error {
	...
	f.HeaderComment("generated by 'threeport-sdk gen' for API client boilerplate' - do not edit")
	f.ImportAlias("github.com/threeport/threeport/pkg/util/v0", "util")
	f.ImportAlias("github.com/threeport/threeport/pkg/client/v0", "client_v0")
	...
	// API version declaration constant
	v := NewFile(cc.PackageName)
	v.HeaderComment("generated by 'threeport-sdk gen' for API client boilerplate' - do not edit")
	v.Const().Id("ApiVersion").Op("=").Lit(cc.PackageName)

	// write version declaration code to file
	vFilename := "version_gen.go"
	vFilepath := filepath.Join(clientLibPath(cc.PackageName), vFilename)
	file, err = os.OpenFile(vFilepath, os.O_CREATE|os.O_WRONLY|os.O_TRUNC, 0644)
	if err != nil {
		return fmt.Errorf("failed to open file to write generated code for model client version declaration: %w", err)
	}
	defer file.Close()
	if err := v.Render(file); err != nil {
		return fmt.Errorf("failed to render generated source code for model client version declaration: %w", err)
	}
	fmt.Printf("code generation complete for %s client version declaration\n", cc.ControllerDomainLower)

	return nil
}

// ExtensionClientLib generates the client library code for the API models in a model
// file for a threeport extension.
func (cc *ControllerConfig) ExtensionClientLib(modulePath string) error {
	pluralize := pluralize.NewClient()
	f := NewFile(cc.PackageName)
	f.HeaderComment("generated by 'threeport-sdk gen' for API client boilerplate' - do not edit")
	f.ImportAlias("github.com/threeport/threeport/pkg/util/v0", "tputil")
	...
	for _, mc := range cc.ModelConfigs {
		...
		f.Func().Id(getAllFuncName).Params(
			...
			Op("*").Index().Qual(
				fmt.Sprintf("%s/pkg/api/%s", modulePath, cc.PackageName),
				mc.TypeName,
			),
			Error(),
		)).Block(
	...
	clientLibPath := clientLibPath(cc.PackageName)
	if _, err := os.Stat(clientLibPath); errors.Is(err, os.ErrNotExist) {
		if err := os.MkdirAll(clientLibPath, 0755); err != nil {
			return fmt.Errorf("could not create directores for model handlers: %s, %w", clientLibPath, err)
		}
	}

	// write code to file
	genFilename := fmt.Sprintf("%s_gen.go", sdk.FilenameSansExt(cc.ModelFilename))
	genFilepath := filepath.Join(clientLibPath, genFilename)
	file, err := os.OpenFile(genFilepath, os.O_CREATE|os.O_WRONLY|os.O_TRUNC, 0644)
	if err != nil {
		return fmt.Errorf("failed to open file to write generated code for model client library: %w", err)
	}
	...
	// write version declaration code to file
	vFilename := "version_gen.go"
	vFilepath := filepath.Join(clientLibPath, vFilename)
	file, err = os.OpenFile(vFilepath, os.O_CREATE|os.O_WRONLY|os.O_TRUNC, 0644)
	if err != nil {
		return fmt.Errorf("failed to open file to write generated code for model client version declaration: %w", err)
	}

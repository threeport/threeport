...
func (cc *ControllerConfig) ModelHandlers() error {
	...
	f.ImportAlias("github.com/threeport/threeport/pkg/api-server/v0", "apiserver")
...
func (cc *ControllerConfig) ExtensionModelHandlers(modulePath string) error {
	...
	modelHandlerPath := apiHandlersPath(cc.ApiVersion)
	if _, err := os.Stat(modelHandlerPath); errors.Is(err, os.ErrNotExist) {
		if err := os.MkdirAll(modelHandlerPath, 0755); err != nil {
			return fmt.Errorf("could not create directores for model handlers: %s, %w", modelHandlerPath, err)
		}
	}

	// write code to file
	genFilename := fmt.Sprintf("%s_gen.go", sdk.FilenameSansExt(cc.ModelFilename))
	genFilepath := filepath.Join(modelHandlerPath, genFilename)
	file, err := os.OpenFile(genFilepath, os.O_CREATE|os.O_WRONLY|os.O_TRUNC, 0644)
	if err != nil {
		return fmt.Errorf("failed to open file to write generated code for model handlers: %w", err)
	}

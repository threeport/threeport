	...
	f.Func().Params(
		...
		g.If(List(Id("_"), Err()).Op(":=").Qual(
			...
			Return(Qual("fmt", "Errorf").Call(
				Lit(fmt.Sprintf(
					"failed to create %s extension namespace: %%w",
					extensionNameKebab,
				)),
				Err(),
			)),
		...
		extensionDbName := fmt.Sprintf("%s_threeport_api", extensionNameSnake)
...
GRANT ALL ON DATABASE %[1]s TO threeport;`, extensionDbName)).Op(",").Line(),
		...
		g.If(List(Id("_"), Err()).Op(":=").Qual(
			...
			Return(Qual("fmt", "Errorf").Call(
				Lit(fmt.Sprintf(
					"failed to create %s DB initialization configmap: %%w",
					extensionNameKebab,
				)),
				Err(),
			)),
						...
						Lit("spec"): Map(String()).Interface().Values(Dict{
							...
							Lit("initContainers"): Index().Interface().Values(
								...
								Line().Map(String()).Interface().Values(Dict{
									...
									Lit("image"):           Lit("localhost:5001/threeport-database-migrator:dev"),
									Lit("imagePullPolicy"): Lit("IfNotPresent"),
		...
		g.If(List(Id("_"), Err()).Op(":=").Qual(
			...
			Return(Qual("fmt", "Errorf").Call(
				Lit(fmt.Sprintf(
					"failed to create %s API deployment: %%w",
					extensionNameKebab,
				)),
				Err(),
			)),
		...
		g.If(List(Id("_"), Err()).Op(":=").Qual(
			...
			Return(Qual("fmt", "Errorf").Call(
				Lit(fmt.Sprintf(
					"failed to create %s controller deployment: %%w",
					extensionNameKebab,
				)),
				Err(),
			)),
	...
	f.Func().Id("copySecret").Params(
		...
		List(Id("mapping"), Err()).Op(":=").Id("restMapper").Dot("RESTMapping").Call(Id("secretGK"), Id("secretGVR").Dot("Version")),
		If(Err().Op("!=").Nil()).Block(
			Return(Qual("fmt", "Errorf").Call(Lit("failed to get RESTMapping for Secret resource: %w"), Err())),
		),
		Line(),

		Id("targetSecretResource").Op(":=").Id("dynamicClient").Dot("Resource").Call(Id("mapping").Dot("Resource")).Dot("Namespace").Call(Id("targetNamespace")),
		List(Id("_"), Err()).Op("=").Id("targetSecretResource").Dot("Get").Call(Qual("context", "TODO").Call(), Id("secretName"), Qual("k8s.io/apimachinery/pkg/apis/meta/v1", "GetOptions").Values()),
		If(Err().Op("==").Nil()).Block(
		...
		List(Id("_"), Err()).Op("=").Id("targetSecretResource").Dot("Create").Call(Qual("context", "TODO").Call(), Id("secret"), Qual("k8s.io/apimachinery/pkg/apis/meta/v1", "CreateOptions").Values()),
		If(Err().Op("!=").Nil()).Block(
			Return(Qual("fmt", "Errorf").Call(Lit("failed to create Secret in namespace '%s': %w"), Id("targetNamespace"), Err())),
		),
		Line(),

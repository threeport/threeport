...
func GenDatabaseInit(gen *gen.Generator) error {
	...
	f.Func().Id("Init").Params(
		...
		Id("dsn").Op(",").Err().Op(":=").Id("GetDsn").Call(),
		If().Err().Op("!=").Nil().Block(
	...
	f.Comment("env vars is not set.")
	f.Func().Id("GetDsn").Params().Params(String(), Error()).Block(
		Var().Id("dbEnvErrors").Qual(
			"github.com/threeport/threeport/pkg/util/v0",
			"MultiError",
		),
		Line(),
		Id("requiredDbEnvVars").Op(":=").Map(String()).String().Values(Dict{
			Lit("DB_HOST"):     Lit(""),
			Lit("DB_PORT"):     Lit(""),
			Lit("DB_USER"):     Lit(""),
			Lit("DB_NAME"):     Lit(""),
		...
		Line(),
		For(List(Id("env"), Op("_")).Op(":=").Range().Id("requiredDbEnvVars")).Block(
			List(Id("val"), Id("ok")).Op(":=").Qual("os", "LookupEnv").Call(Id("env")),
			If(Op("!").Id("ok")).Block(
				Id("dbEnvErrors").Dot("AppendError").Call(Qual("fmt", "Errorf").Call(Lit("missing required environment variable: %s"), Id("env"))),
		...
		Line(),
		Id("dsn").Op(":=").Qual("fmt", "Sprintf").Call(
			Line().Lit("host=%s port=%s user=%s dbname=%s sslmode=%s sslrootcert=%s/ca.crt sslcert=%[6]s/client.threeport.crt sslkey=%[6]s/client.threeport.key TimeZone=UTC"),
			Line().Id("requiredDbEnvVars").Index(Lit("DB_HOST")),
			Line().Id("requiredDbEnvVars").Index(Lit("DB_PORT")),
			Line().Id("requiredDbEnvVars").Index(Lit("DB_USER")),
		...
		Line(),
		Return(Id("dsn"), Id("dbEnvErrors").Dot("Error").Call()),
	)
